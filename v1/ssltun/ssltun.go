...
import (
	...
	"github.com/fehmicorp/pkg/v1/dns"
...
func SetupTunnel(pool *dns.Pool, listenPort string, gwIp string, routePool []string) {
	cfg := water.Config{
...
func handleClient(conn net.Conn, iface *water.Interface, pool *dns.Pool) {
	defer conn.Close()
